...
import { useGetDegrees, useGetSpecialities } from 'src/hooks/useLookups'
...
import { useAddEducation, useDeleteEducation, useGetEducationById, useUpdateEducation } from 'src/hooks/useUsers'
...
export default function AddEducation() {
  ...
  const { data: experience, isLoading: isFetchingExperience } = useGetEducationById({
    ...
    onSuccess: (data: any) => {
      ...
      setSelectedPlace({ name: data.institution_name });
    }
  });

  const { mutate: createMutation, isLoading: isCreating } = useAddEducation(
    (data: any) => {
  ...
  const { mutate: updateMutation, isLoading: isUpdating } = useUpdateEducation(
    (data: any) => {
      if (data?.status_code == 200) {
  ...
  const { mutate: deleteMutation, isLoading: isDeleting } = useDeleteEducation(
    (data: any) => {
      if (data?.status_code == 200) {
        setDeleteOpen(false);
  ...
  const { data: degrees } = useGetDegrees({
    onSuccess: (data: any) => console.log('degrees', data),
    onError: (err: any) => console.log('error fetching degrees', err),
  });

  const selectPlace = (place: any) => {
    formRef.current.setFieldValue('institution_name', place.name);
    setSelectedPlace(place);
  }

  const handleSubmit = (values: any) => {
    if (checked) delete values.end_date;
    values.institution_name = selectedPlace.name;
